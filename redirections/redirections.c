...
/*   Updated: 2017/06/19 19:51:26 by apetitje         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	find_first(t_token *line)
{
	...
	if (ft_isdigit((*line).word[0]))
		...
		first = 1;
	if (first < 0)
		ft_dprintf(2, "21sh: file descriptor error\n");
	return (first);
}
...
static void	redir_stdin_stderr(t_token *line, t_list **redirs)
{
	if ((*line).type == TOKEN_REDIR_R)
	{
		...
		{
			add_to_redir_list(line + 1, redirs, 1, REDIR_TO);
			add_to_redir_list(line + 1, redirs, 2, REDIR_TO);
		}
	}
	else if ((*line).word[2] == '&')
	{
	...
	{
		add_to_redir_list(line + 1, redirs, 1, REDIR_APPEND);
		add_to_redir_list(line + 1, redirs, 2, REDIR_APPEND);
	}
}
