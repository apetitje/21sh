...
/*   Updated: 2017/06/21 13:36:16 by apetitje         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int			do_pipe(t_ast *root, int *stdin_save,
		int code, t_pipe_lists *pipe_lists)
{
	int		pipe_fds[2];
	int		in_bk;
	int		out_bk;
	...
	add_to_fd_list(pipe_fds, &(pipe_lists->fd_list));
	dup2(*stdin_save, STDIN_FILENO);
	if (code)
		dup2(pipe_fds[1], STDOUT_FILENO);
	if (ast_sub_command(root, code))
	{
		...
		t_pipe_lists *pipe_lists)
{
	if (root)
	{
		if (root->left && ((t_token *)root->left->item)->type == TOKEN_PIPE)
			if (pipe_com(root->left, 1, stdin_save, pipe_lists))
				return (1);
				...
				&& ((t_token *)root->item)->type == TOKEN_PIPE)
		{
			...
			if (pipe_sub_com(root, code, stdin_save, pipe_lists))
				return (1);
		}
		if (root->right)
			pipe_com(root->right, 1, stdin_save, pipe_lists);
	}
